<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-2xl text-gray-800 leading-tight">Create Expense Category</h2>
    </x-slot>

    <div class="py-12">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl rounded-lg">
                <form action="{{ route('expense-categories.store') }}" method="POST" class="p-8">
                    @csrf
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                            <div class="flex items-start">
                                <input type="checkbox" name="requires_approval" id="requires_approval" value="1" {{ old('requires_approval') ? 'checked' : '' }}
                                    class="w-4 h-4 text-yellow-600 border-gray-300 rounded focus:ring-yellow-500 mt-1">
                                <div class="ml-3">
                                    <label for="requires_approval" class="font-medium text-gray-900">Requires Approval</label>
                                    <p class="text-sm text-gray-600">Check if expenses in this category need Boss/Archid approval</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center justify-end space-x-4 pt-6 border-t border-gray-200">
                            ...
                            <button type="submit" class="px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-lg shadow-md">Create Category</button>
                        </div>
                    </div>
                </form>
            </div>
