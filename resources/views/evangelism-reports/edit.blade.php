                        ...
                        <div>
                            ...
                            @if($churches->count() > 0)
                                <select name="church_id" required class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:border-brand-500 focus:ring-brand-500">
                                    ...
                                    @foreach($churches as $church)
                                        <option value="{{ $church->id }}" {{ $evangelismReport->church_id == $church->id || (old('church_id', $churches->count() === 1 ? $churches->first()->id : null) == $church->id) ? 'selected' : '' }}>
                                            {{ $church->name }}
                                        </option>
                                    @endforeach
                                </select>
                            @else
                                <div class="p-4 mb-4 text-sm text-red-800 rounded-lg bg-red-50">
                                    You do not have any assigned churches.
                                </div>
